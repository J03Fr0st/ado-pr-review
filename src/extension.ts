...
export async function activate(context: vscode.ExtensionContext): Promise<void> {
    try {
        ...
        const authenticationService = new AuthenticationService(context, context.secrets);
        const telemetryService = TelemetryService.getInstance('azure-devops-pr-reviewer', '1.0.0');
        const apiClient = new AzureDevOpsApiClient(authenticationService, configurationService, context);
        const pullRequestService = new PullRequestService(apiClient, configurationService, context);
        const commentService = new CommentService(apiClient, configurationService, context);
        ...
        const disposable = vscode.commands.registerCommand('azureDevOps.openPullRequest', async (item) => {
            try {
        ...
        console.error('Error activating Azure DevOps PR Reviewer extension:', error);
        vscode.window.showErrorMessage('Failed to activate Azure DevOps PR Reviewer extension');
        throw error;
    }
}
