...
class RepositoryTreeItem extends BaseTreeItem {
    constructor(
        ...
        super(
            ...
            repository.name,
            pullRequests.length > 0 ? vscode.TreeItemCollapsibleState.Collapsed : vscode.TreeItemCollapsibleState.None
        );

        this.description = `${pullRequests.length} pull request${pullRequests.length !== 1 ? 's' : ''}`;
...
class PullRequestTreeItem extends BaseTreeItem {
    ...
    private getPullRequestDescription(): string {
        ...
        const created = new Date(this.pullRequest.creationDate).toLocaleDateString();
    ...
    private getPullRequestTooltip(): string {
        ...
        return `Status: ${status}${draft}\nMerge Status: ${mergeStatus}\nCreated: ${new Date(this.pullRequest.creationDate).toLocaleString()}`;
    }
...
class LoadingTreeItem extends BaseTreeItem {
    constructor() {
        super('loading', TreeItemType.LOADING, 'Loading...', vscode.TreeItemCollapsibleState.None);
...
class ErrorTreeItem extends BaseTreeItem {
    constructor(private readonly errorMessage: string) {
        super('error', TreeItemType.ERROR, 'Error loading pull requests', vscode.TreeItemCollapsibleState.None);
...
class NoPullRequestsTreeItem extends BaseTreeItem {
    constructor() {
        super('no-prs', TreeItemType.NO_PULL_REQUESTS, 'No pull requests found', vscode.TreeItemCollapsibleState.None);
 ...
 */
export class PullRequestTreeProvider implements vscode.TreeDataProvider<BaseTreeItem> {
    private readonly _onDidChangeTreeData: vscode.EventEmitter<BaseTreeItem | undefined | null | void> = new vscode.EventEmitter<BaseTreeItem | undefined | null | void>();
    ...
    async getChildren(element?: BaseTreeItem): Promise<BaseTreeItem[]> {
        ...
        if (element instanceof RepositoryTreeItem) {
            return element.pullRequests.map(pr => new PullRequestTreeItem(element.repository, pr));
        }
    ...
    getParent(element: BaseTreeItem): vscode.ProviderResult<BaseTreeItem> {
        if (element instanceof PullRequestTreeItem) {
            return this.treeItems.find(item =>
                item instanceof RepositoryTreeItem && item.repository.id === element.repository.id
            );
        }
        return null;
    }
    ...
    private async loadPullRequests(): Promise<void> {
        try {
            ...
            const repositoryMap = new Map<string, { repository: GitRepository; pullRequests: PullRequest[] }>();
            ...
            // Create tree items
            this.treeItems = Array.from(repositoryMap.values()).map(({ repository, pullRequests }) => {
                // Sort pull requests by creation date (newest first)
                pullRequests.sort((a, b) => new Date(b.creationDate).getTime() - new Date(a.creationDate).getTime());
            ...
            this.treeItems = [new ErrorTreeItem(error instanceof Error ? error.message : 'Unknown error')];
        } finally {
            this._onDidChangeTreeData.fire();
        }
    }
    ...
    getPullRequestById(pullRequestId: number): PullRequest | undefined {
        for (const item of this.treeItems) {
            if (item instanceof RepositoryTreeItem) {
                const pr = item.pullRequests.find(p => p.pullRequestId === pullRequestId);
                if (pr) {
                    return pr;
                }
            }
    ...
    getActivePullRequests(): PullRequest[] {
        ...
        for (const item of this.treeItems) {
            if (item instanceof RepositoryTreeItem) {
                activePRs.push(...item.pullRequests.filter(pr => pr.status === 'active'));
            }
        }
        return activePRs;
    }
